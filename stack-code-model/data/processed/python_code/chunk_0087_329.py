package com.kyo.media.simpleVideo
{
	 ...
	 * 简单的视频播放类
	 * @author LiuSheng
	 * @created 18-Sep-2017 14:43:46
	 */
	public class SimpleVideo extends Video implements ICommonLoader
	{
		public function SimpleVideo(needReloading:Boolean = false, needLoop:Boolean = false){
			super();
...
//		public static const TIME_DELAY_TO_FINISH:Number = 0.2;// 单位：s
		...
		private var _needLoop:Boolean = false;
		
		private var _waitingToPauseStatus:int = 0;
		private var _waitingToPausePoint:Number = 0;
...
//			Log.info("SimpleVideo.stopVideo() : seek(0)"); 
		...
		private function reportPlayingStatus(e:TimerEvent=null):void{
			...
			if(_waitingToPauseStatus == 2)
			{
				//if(ns.time >= _waitingToPausePoint)
				if(ns.time > _waitingToPausePoint + 1)
				{
//					ns.pause();
					pauseVideo();
					_waitingToPauseStatus = 0;
				}
			}
		}
...
//					//isPlaying = false;
				...
				case "NetStream.Seek.Notify":
					...
					_isPlaying = true;
					dispatchEvent(new SimpleVideoEvent(SimpleVideoEvent.ON_SEEKING));
					if(_waitingToPauseStatus == 1)
					{
						_waitingToPauseStatus = 2;
					}
					break;
				case "NetStream.SeekStart.Notify":
					trace("NetStream.SeekStart.Notify");
					_isPlaying = false;
					_isVideoPlayStarted = false;
					dispatchEvent(new SimpleVideoEvent(SimpleVideoEvent.ON_SEEK_START));
					break;
				case "NetStream.Seek.Complete":
					_isPlaying = true;
					_isVideoPlayStarted = true;
					dispatchEvent(new SimpleVideoEvent(SimpleVideoEvent.ON_SEEK_COMPLETE));
...
//				ns.play2();
		...
		public function seekAndPause(seekPoint:Number, pausePoint:Number):void
		{
			seek(seekPoint);
			_waitingToPauseStatus = 1;
			_waitingToPausePoint = pausePoint;
		}
