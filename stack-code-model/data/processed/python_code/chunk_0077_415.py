package org.aswing.components.windows
{
	...
	public class JFrameTitleBar extends Container implements FrameTitleBar, UIResource
	{
		...
		public function JFrameTitleBar(onlyClosable:Boolean = false)
		{
			super();
			titleEnabled = true;
			buttonMode = true;
			...
			labelPane.setBorder(new EmptyBorder(null, new Insets(-3, 7))); //make label y offset -3, x offset 7
			...
			btnpP.setBorder(new EmptyBorder(null, new Insets(-1, 0, 0, 7))); // right offset 7
		...
		public function setClosableOnly(b:Boolean):void
		{
			setIconifiedButton(b ? null : createIconifiedButton());
			setMaximizeButton(b ? null : createMaximizeButton());
			setRestoreButton(b ? null : createRestoreButton());
			updateUI();
		}
		...
		private function __stateChanged(e:InteractiveEvent):void
		{
			...
			if (state != JFrame.ICONIFIED && state != JFrame.NORMAL && state != JFrame.MAXIMIZED_HORIZ && state != JFrame.MAXIMIZED_VERT && state != JFrame.MAXIMIZED)
			{
				state = JFrame.NORMAL;
			}
