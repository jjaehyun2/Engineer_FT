package away3d.materials.passes
{
	 ...
	 */
	public class OrthoSegmentPass extends MaterialPassBase
	{
		protected static const ONE_VECTOR:Vector.<Number> = Vector.<Number>([ 1, 1, 1, 1 ]);
		protected static const FRONT_VECTOR:Vector.<Number> = Vector.<Number>([ 0, 0, 1, 1 ]);
		 ...
		 */
		public function OrthoSegmentPass(thickness:Number)
		{
		...
		override arcane function activate(stage3DProxy:Stage3DProxy, camera:Camera3D):void
		{
			...
			if (stage3DProxy.scissorRect)
				_constants[0] = _thickness*0.5*(camera.lens as OrthographicLens).projectionHeight/stage3DProxy.scissorRect.height;
			else
				_constants[0] = _thickness*0.5*(camera.lens as OrthographicLens).projectionHeight/stage3DProxy.height;
