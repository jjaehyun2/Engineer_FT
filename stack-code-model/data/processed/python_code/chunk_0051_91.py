...
package starling.styles
{
	...
	public class MeshStyle extends EventDispatcher
	{
		...
		public function updateEffect( effect : MeshEffect, state : RenderState ) : void
		{
			...
			effect.mvpMatrix3D.copyFrom( state.mvpMatrix3D );
			var invalidProgramVariantName : Boolean = false;
			if ( effect.alpha != state.alpha )
			{
				effect.alpha = state.alpha;
				invalidProgramVariantName = true;
			}
			if ( effect.tinted != _vertexData.tinted )
			{
				effect.tinted = _vertexData.tinted;
				invalidProgramVariantName = true;
			}
			if ( invalidProgramVariantName )
			{
				effect.validProgramVariantName();
			}
		}
