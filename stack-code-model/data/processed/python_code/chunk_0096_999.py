package com.rpg.astar.silz
{
		...
		public class SilzAstar{
			 ...
			 * 地图显示尺寸
			 */ 
			private var _cellWidth:int = 6;
			
			private var _cellHeight:int = 3;
			/**
			 * 路径显示器
			 */ 
			private var path:Sprite = new Sprite();
			private var path2:Sprite = new Sprite();
			/**
			 * 地图显示器
			 */ 
			...
			/**
			 * @param		xnow	当前格子X(世界坐标)
			 * @param		ynow	当前格子Y(世界坐标)
			 * @param		xpos	目标格子X(世界坐标)
			 * @param		ypos	目标格子Y(世界坐标)
			 */ 
			public function find(xnow:uint,ynow:uint,xpos:uint,ypos:uint):Array 
			{
				...
				if (astar.findPath()){
					...
					{
						time = getTimer() - time;
						trace(time + "ms   优化前格子数length:" + astar.path.length);
						trace(astar.floydPath);
						path.graphics.clear();
						path2.graphics.clear();
						
						//优化后路径描点
						for (var i:int = 0; i < astar.floydPath.length; i++){
							var p:Node = astar.floydPath[i];
							path.graphics.lineStyle(0, 0xff0000);
							path.graphics.drawCircle((p.x + 0.5) * _cellWidth, (p.y + 0.5) * _cellHeight, 4);
						...
						//优化前路径描点
						for (var j:int = 0; j < astar.path.length; j++){
							var p2:Node = astar.path[j];
							path2.graphics.lineStyle(0, 0x00ff00);
							path2.graphics.drawCircle((p2.x + 0.5) * _cellWidth, (p2.y + 0.5) * _cellHeight, 2);
							
							path2.graphics.lineStyle(0, 0xff0000, 0.5);
							path2.graphics.moveTo(xnow,ynow);
						}
					}
			...
			{
				var rows:int = data[0].length;
				var cols:int = data.length;
				...
				for(py=0;py<rows;py++)
				{
					for(px=0;px<cols;px++)
					{
						_grid.setWalkable(px,py,data[px][py]!=1);
					}
				}
			...
			{
				image.bitmapData = new BitmapData(_grid.numCols * _cellWidth, _grid.numRows * _cellHeight, false, 0xffffff);
				for (var i:int = 0; i < _grid.numCols; i++)
				{
					for (var j:int = 0; j < _grid.numRows; j++)
					{
						...
						if (!node.walkable)
						{
							image.bitmapData.fillRect(new Rectangle(i * _cellWidth, j * _cellHeight, _cellWidth, _cellHeight), getColor(node));
						}
					}
				}
			}
