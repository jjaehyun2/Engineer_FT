...
package devoron.aswing3d.plaf.basic
{
	...
	public class BasicColorMixerUI extends BaseComponentUI
	{
		...
		private function getMixerPaneSize():IntDimension
		{
			...
			hss.change(crm, crm);
			ls.change(crm, crm);
			var size:IntDimension = new IntDimension(hss.width + getHS2LGap() + ls.width, Math.max(hss.height, ls.height));
		...
		private function paintHSMC():void
		{
			...
			g.drawLine(new Pen(ASColor.GRAY, thickness), offset + thickness / 2, offset + thickness / 2, offset + w - thickness, offset + thickness / 2);
			g.drawLine(new Pen(ASColor.GRAY, 1), offset + thickness / 2, offset + thickness / 2, offset + thickness / 2, offset + h - thickness);
		...
		private function paintHSPosMC():void
		{
			...
			g.drawLine(new Pen(new ASColor(0xFFFFFF, 0.8), 2), -6, 0, -3, 0);
			g.drawLine(new Pen(new ASColor(0xFFFFFF, 0.8), 2), 6, 0, 3, 0);
			g.drawLine(new Pen(new ASColor(0xFFFFFF, 0.8), 2), 0, -6, 0, -3);
			g.drawLine(new Pen(new ASColor(0xFFFFFF, 0.8), 2), 0, 6, 0, 3);
		}
		...
		private function paintLMCWithHS(hh:Number, ss:Number):void
		{
			...
			g.drawLine(new Pen(ASColor.GRAY, thickness), x + thickness / 2, y + thickness / 2, x + w - thickness, y + thickness / 2);
			g.drawLine(new Pen(ASColor.GRAY, 1), x + thickness / 2, y + thickness / 2, x + thickness / 2, y + h - thickness);
			x += thickness;
			y += thickness;
			w = getLStripWidth();
			h = lSize.height;
		...
		private function paintLPosMC():void
		{
			...
			g.fillPolygon(new SolidBrush(new ASColor(0xFFFFFF,0.8)), [new IntPoint(0, 0), new IntPoint(4, -4), new IntPoint(4, 4)]);
			LPosMC.x = getHSSize().width + getMCsMarginSize() + getColorRectMargin() * 2 + getHS2LGap() + getLStripWidth() + 1;
		}
		...
		{
			return new JColorTextField("000000", 6)
		}
		...
		private function createAAdjuster():JAdjuster
		{
			...
			adjuster.setValueTranslator(function(value:Number):String
				{
