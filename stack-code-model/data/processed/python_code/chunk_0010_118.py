...
*--------------------------------------------------------------------*
CLASS lcl_cut_subclass DEFINITION FINAL INHERITING FROM /usi/cl_bal_em_base FOR TESTING.
  PUBLIC SECTION.
...
CLASS lcl_unit_test IMPLEMENTATION.
  METHOD test_refuse_unbound_exception.
    DATA: cut               TYPE REF TO lcl_cut_subclass,
          unbound_reference TYPE REF TO cx_root.

    TRY.
        CREATE OBJECT cut
          EXPORTING
            i_exception = unbound_reference.
  ...
  METHOD assert_container_in_collection.
    ...
    LOOP AT data_containers ASSIGNING <data_container>.
      IF <data_container> EQ i_data_container.
        RETURN.
      ENDIF.
    ENDLOOP.
