package devoron.components.filechooser.autocomplete
{
	...
	public class AutocompleteManager
	{
		...
		private var _targetTextField:JTextField;
		private var pathCB:DSComboBox;
		public var suggestionActive:Boolean = false;
		public var ready:Boolean = false;
		
		public function AutocompleteManager(targetTextField:JTextField/* = null*/, pathCB:DSComboBox)
		{
			this.pathCB = pathCB;
		...
		public function complete():void
		{
			...
			var nextChar:String = _targetTextField.getText().charAt(_targetTextField.getTextField().caretIndex);
			if (_targetTextField.getTextField().caretIndex < _targetTextField.getText().length && nextChar != "" && nextChar != " ")
			{
				return;
			}
			
			//we only complete single words, so start caret is the beginning of the word the caret is currently in
			var firstIndex:int = TextUtils.getFirstIndexOfWordAtCaretIndex(_targetTextField.getTextField());
			var str:String = _targetTextField.getText().substr(firstIndex, _targetTextField.getTextField().caretIndex);
			...
			{
				//(pathCB.getModel() as VectorListModel).clear();
				//(pathCB.getModel() as VectorListModel).appendAll();
				
				//Sort of a brutal divide and conquer strategy here. Someone smarter take a look?
				var _originalText:String = _targetTextField.getText();
				var originalCaretIndex:int = _targetTextField.getTextField().caretIndex;
				var currentWord:String = TextUtils.getWordAtCaretIndex(_targetTextField.getTextField());
				...
				_targetTextField.setText(wordSplit.join(" "));
		...
		public function get targetTextField():JTextField
		{
			return _targetTextField;
		}
		
		public function set targetTextField(value:JTextField):void
		{
			try
			{
