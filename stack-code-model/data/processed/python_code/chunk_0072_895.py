...
package starling.textures
{
    ...
    public class RenderTexture extends SubTexture
    {
         ...
         */
        public function RenderTexture( name:String, width:int, height:int, persistent:Boolean = true, scale:Number = -1, format:String = "bgraPacked4444", repeat:Boolean = false )
        {
//			this.name = name;
        ...
        private function get supportsNonPotDimensions():Boolean
        {
            ...
            if (support == null)
            {
                if (target.profile != "baselineConstrained" && "createRectangleTexture" in context)
                {
                    ...
                    try
                    {
                        texture = context["createRectangleTexture"](2, 3, "bgraPacked4444", true);
