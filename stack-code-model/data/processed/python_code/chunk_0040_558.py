package devoron.aswing3d.plaf.basic
{
	...
	public class BasicClosableTabbedPaneUI extends BasicTabbedPaneUI
	{
		...
		override protected function setTabProperties(header:Tab, i:int):void
		{
			...
			ClosableTab(header).getCloseButton().setEnabled(JClosableTabbedPane(tabbedPane).isCloseEnabledAt(i) && tabbedPane.isEnabledAt(i));
		}
		...
		{
			if ((prevButton.hitTestMouse() || nextButton.hitTestMouse()) && (prevButton.isShowing() && nextButton.isShowing()))
			{
				return;
			}
