/*
	...
	Tool: Sidelaser (developed by Daniel Brendel)
	Version: 0.1
...
const int BULLET_SIDE_LEFT = 1;
const int BULLET_SIDE_RIGHT = 2;
class CLaserBullet : IScriptedEntity
{
	Vector m_vecPos;
	Model m_oModel;
	Timer m_oLifeTime;
	...
	void Move()
	{
		if (this.m_iSide == BULLET_SIDE_LEFT) {
			this.m_vecPos[0] += this.m_iSpeed;
		} else if (this.m_iSide == BULLET_SIDE_RIGHT) {
			this.m_vecPos[0] -= this.m_iSpeed;
		}
	}
	...
	{
		if (this.m_iSide == BULLET_SIDE_LEFT) {
			this.m_vecPos = Vector(39, vec[1]);
			this.m_hSprite = R_LoadSprite(g_szToolPath + "beam_left.png", 1, 92, 58, 1, false);
		} else if (this.m_iSide == BULLET_SIDE_RIGHT) {
			this.m_vecPos = Vector(Wnd_GetWindowCenterX() * 2 - 39, vec[1]);
			this.m_hSprite = R_LoadSprite(g_szToolPath + "beam_right.png", 1, 92, 58, 1, false);
		}
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(20, 20), Vector(64, 64));
		this.m_oModel.Initialize2(bbox, this.m_hSprite);
	}
	...
	void OnRelease()
	{
		if (!this.m_oLifeTime.IsElapsed()) {
			CDecalSprite @obj = CDecalSprite();
			Ent_SpawnEntity(@obj, this.m_vecPos);
			
			CExplosion @expl = CExplosion();
			Ent_SpawnEntity(@expl, Vector(this.m_vecPos[0] + 10, this.m_vecPos[1] - 10));
		}
	}
	...
	bool NeedsRemoval()
	{
		return (this.m_oLifeTime.IsElapsed()) || (this.m_bRemove);
	}
...
class CGun : IScriptedEntity
{
	Vector m_vecPosLeft;
	Vector m_vecPosRight;
	Vector m_vecSelSize;
	Model m_oModel;
	SpriteHandle m_hSpriteLeft;
	SpriteHandle m_hSpriteRight;
	Timer m_tmrMovement;
	int m_iSpeed;
	Timer m_tmrShoot;
	SoundHandle m_hShoot;
	
	//Move guns
	void Move()
	{
		this.m_vecPosLeft[1] += this.m_iSpeed;
		this.m_vecPosRight[1] += this.m_iSpeed;
	}
	
	//Shoot bullets left or right
	void Shoot()
	{
		bool bCurShootSide = Util_Random(0, 2) == 1;
		int side = (bCurShootSide) ? BULLET_SIDE_LEFT : BULLET_SIDE_RIGHT;
		Vector vec = (bCurShootSide) ? this.m_vecPosLeft : this.m_vecPosRight;
		
		CLaserBullet @bullet = CLaserBullet();
		bullet.SetSide(side);
		Ent_SpawnEntity(@bullet, vec);
		
		S_PlaySound(this.m_hShoot, 10);
	}
	
	CGun()
    {
		this.m_vecSelSize = Vector(0, 0);
		this.m_vecPosLeft = Vector(0, 0);
		this.m_vecPosRight = Vector(Wnd_GetWindowCenterX() * 2 - 39, 0);
		this.m_iSpeed = 10;
    }
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hSpriteLeft = R_LoadSprite(g_szToolPath + "gun_left.png", 0, 39, 50, 0, false);
		this.m_hSpriteRight = R_LoadSprite(g_szToolPath + "gun_right.png", 0, 39, 50, 0, false);
		this.m_hShoot = S_QuerySound(g_szToolPath + "laser.wav");
	...
	void OnProcess()
	{
		...
		if (this.m_tmrShoot.IsElapsed()) {
			this.m_tmrShoot.SetDelay(Util_Random(400, 650));
			this.m_tmrShoot.Reset();
			this.Shoot();
		}
	}
	...
	void OnDrawOnTop()
	{
		R_DrawSprite(this.m_hSpriteLeft, this.m_vecPosLeft, 0, 0.0, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
		R_DrawSprite(this.m_hSpriteRight, this.m_vecPosRight, 0, 0.0, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
	}
	...
	bool NeedsRemoval()
	{
		return (this.m_vecPosLeft[1] >= Wnd_GetWindowCenterY() * 2 + 50) && (this.m_vecPosRight[1] >= Wnd_GetWindowCenterY() * 2 + 50);
	}
...
{
	CGun @gun = CGun();
	Ent_SpawnEntity(@gun, Vector(0, 0));
}
