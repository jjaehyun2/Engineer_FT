/* From http://flexdevtips.blogspot.com/2009/04/tree-icon-spinner-not-using-animated.html */
...
package util
{
	...
	[Style(name="spinnerShadowColor1", type="uint", format="Color", inherit="no")]
	[Style(name="spinnerShadowColor2", type="uint", format="Color", inherit="no")]
	...
	public class Spinner extends UIComponent
	{
		...
		private static const POSITIONS:int = 8; //8
		...
		public function start():void {
			if (!timer.running) {
				// Reset the spinner and repaint it immediately.
				//currentPosition = 0;
				timerHandler(null);
		...
		protected function drawSpinner(g:Graphics, w:Number, h:Number):void {
			...
			var shadow1:uint = uint(getStyle("spinnerShadowColor1"));
			var shadow2:uint = uint(getStyle("spinnerShadowColor2"));
			...
			if (drawLines || drawCircles) {
				...
				while (angle < MAX_ANGLE) {
					...
					var color:uint = 0;
					var thisRadius:int = smallRadius;
					
					if (count == currentPosition)
						color = highlight;
					else if ((count + 1) % POSITIONS == currentPosition)
					{
						color = shadow1;
						thisRadius -= 1;
					}
					else if ((count + 2) % POSITIONS == currentPosition)
					{
						color = shadow2;
						thisRadius -= 2;
					}
					...
					if (drawLines) {
						...
						drawCircle(g, color, x1, y1, thisRadius, spinnerAlpha);
					}
		...
		protected function drawGradientCircle(g:Graphics, color:uint, highlight:uint, 
			...
			if ((radius > 0) && (thickness > 0) && (circleAlpha > 0)) {
	            ...
	            g.lineGradientStyle(GradientType.LINEAR, [highlight, color], [circleAlpha, circleAlpha], [0, 75], matrix); //96
				g.drawCircle(midX, midY, radius);
			}
		}
