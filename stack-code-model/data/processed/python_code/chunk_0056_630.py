package uieditor.editor.ui.tabpanel
{
	...
	public class LayoutTab extends LayoutGroup
	{
		...
		private function onSelectChange( event : Event ) : void
		{
			...
			_list.selectedIndex = _documentEditor.selectedIndex;
		...
		private function updateList( scrollToIndex : Boolean = true ) : void
		{
			...
			if ( item != null )
			{
				var oldIndex : int = _list.dataProvider.getItemIndex( item );
				if ( _list.selectedIndex != oldIndex )
				{
					_list.selectedIndex = oldIndex;
					if ( scrollToIndex )
					{
						_list.scrollToDisplayIndex( oldIndex );
					}
				}
			}
