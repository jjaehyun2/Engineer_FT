...
package org.aswing.components.windows
{
	...
	public class JFrame extends JWindow
	{
		...
		private var closeKeyStroke:KeyType;
		...
		public function setClosable(b:Boolean):void
		{
			if (closable != b)
			{
				...
				{
					if (closable)
						keyManager.registerKeyAction(closeKeyStroke, tryToClose);
					else
		...
		{
			return ((state & MAXIMIZED_HORIZ) == MAXIMIZED_HORIZ) || ((state & MAXIMIZED_VERT) == MAXIMIZED_VERT);
		}
		...
		public function getCloseKeyAction():KeyType
		{
			return closeKeyStroke;
		}
