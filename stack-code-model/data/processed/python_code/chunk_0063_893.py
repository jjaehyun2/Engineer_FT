...
package com.neopets.util.display
{
	...
	public class BroadcasterClip extends MovieClip
	{
		...
		//   VARIABLES
		//--------------------------------------
		protected var _defaultDispatcher:EventDispatcher;
		...
		// defaultDispatcher uses accessors to let sub classes react when the dispatcher is set.
		// This is normally done when the subclass extracts properties from the dispatcher.
		
		public function get defaultDispatcher():EventDispatcher { return _defaultDispatcher; }
		
		public function set defaultDispatcher(dispatcher:EventDispatcher) {
		...
		public function broadcast(type:String,info:Object=null,dispatcher:EventDispatcher=null,
			...
			if(dispatcher != null) {
				...
				if(_defaultDispatcher != null) {
					_defaultDispatcher.dispatchEvent(transmission);
				} else {
