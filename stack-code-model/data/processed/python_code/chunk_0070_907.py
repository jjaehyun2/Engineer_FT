package assets.components
{
	...
	public class Hpic4vc_Storage_HBAsMediator extends Hpic4vc_BaseMediator
	{
		private var _view:Hpic4vc_Storage_HBAs;
		private var _proxyStorage:Hpic4vc_storage_providerProxy;
		private var count:int = 0;
		
		private static var _logger:ILogger = Log.getLogger("Hpic4vc_Storage_HBAsMediator");
		
		[View]
		/**
		 * The mediator's view.
		 */
		public function get view():Hpic4vc_Storage_HBAs {
			return _view;
		}
		
		/** @private */
		public function set view(value:Hpic4vc_Storage_HBAs):void {
			_view = value;
		...
		override protected function requestData():void {
			if (_contextObject != null) {
				_logger.debug("Requesting HPIC4VC HBA data.");
				_proxyStorage.getHbas(_contextObject.uid, onGettingResult, _contextObject);
			} else {
				_logger.warn("ContextObject is null, hence not requesting HBA data.");
				return;
			}
		}
		...
		private function onGettingResult(event:MethodReturnEvent):void {
			if (_view != null) {
				_logger.debug("Received HPIC4VC HBA data in onGettingResult().");
				...
				_logger.warn("View is null.  Returning from onGettingResult() for HBA data.");
				return;
			}
		}
	}
