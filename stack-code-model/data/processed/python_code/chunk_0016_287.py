...
inheriting from zcl_assert.
  ...
  "Difference between this and light update:
   "it deletes records on db that are not on the result set
   "it also remove entries for tadir records flagged as delete
   "!it will delete everything else on DB that is NOT on the result container.
...
class ltc_results_db implementation.
  ...
  method reset_instances.
    ...
    cut = new zcl_adash_full_db_update(  ).
