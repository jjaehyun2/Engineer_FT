...
package starling.text
{
	...
	public class BitmapFont implements ITextCompositor
	{
		...
		private var _helperSimpleImage : SimpleBitmapImage;
		...
		public function BitmapFont( texture : Texture = null, fontXml : XML = null )
		{
			...
			_helperSimpleImage = new SimpleBitmapImage();
		...
		public function dispose() : void
		{
			...
			if ( _helperSimpleImage != null )
			{
				_helperSimpleImage.dispose();
				_helperSimpleImage = null;
			}
		}
		...
		private function parseFontXml( fontXml : XML ) : void
		{
			...
			_size = parseFloat( fontXml.info.@size ) * invScale;
			_lineHeight = parseFloat( fontXml.common.@lineHeight ) * invScale;
			_baseline = parseFloat( fontXml.common.@base ) * invScale;
			...
			for each ( var charElement : XML in fontXml.chars.char )
			{
				...
				var xOffset : Number = parseFloat( charElement.@xoffset ) * invScale;
				var yOffset : Number = parseFloat( charElement.@yoffset ) * invScale;
				var xAdvance : Number = parseFloat( charElement.@xadvance ) * invScale;

				var region : Rectangle = new Rectangle();
				region.x = parseFloat( charElement.@x ) * invScale + frameX;
				region.y = parseFloat( charElement.@y ) * invScale + frameY;
				region.width = parseFloat( charElement.@width ) * invScale;
				region.height = parseFloat( charElement.@height ) * invScale;
			...
			for each ( var kerningElement : XML in fontXml.kernings.kerning )
			{
				...
				var amount : Number = parseFloat( kerningElement.@amount ) * invScale;
				if ( second in _chars )
		...
		public function fillMeshBatch( meshBatch : IMeshBatch, width : Number, height : Number, text : String,
			format : TextFormat, options : TextOptions = null, isSimpleText : Boolean = false, simpleTextCount : int = -1 ) : void
		{
			...
			if ( !isSimpleText )
			{
				charLocations = arrangeChars( width, height, text, format, options );
			...
			{
				charLocations = arrangeSimpleChars( width, height, text, format, options );
				numChars = charLocations.length;
				if ( numChars > simpleTextCount )
					numChars = simpleTextCount;

				_helperSimpleImage.textColor = format.color;
				_helperSimpleImage.textScale = 1;
				...
				{
					charLocation = charLocations[ i ];
					tex = charLocation.char.texture;
					_helperSimpleImage.textTexture = tex;
					_helperSimpleImage.textWidth = tex.frameWidth;
					_helperSimpleImage.textHeight = tex.frameHeight;
					_helperSimpleImage.textX = charLocation.x;
					_helperSimpleImage.textY = charLocation.y;
					if ( textScale != charLocation.scale )
					{
						_helperSimpleImage.textScale = charLocation.scale;
						textScale = charLocation.scale;
					}
					meshBatch.addMesh( _helperSimpleImage );
				}

				//当前设置的字符数量小于SimpleBitmapText的字符长度，剩余部分使用第一个字符
				if ( isSimpleText && numChars > 0 && simpleTextCount > numChars )
				{
					charLocation = charLocations[ 0 ];
					_helperSimpleImage.textTexture = charLocation.char.texture;
					_helperSimpleImage.textWidth = 0;
					_helperSimpleImage.textHeight = 0;
					_helperSimpleImage.textX = 0;
					_helperSimpleImage.textY = 0;
					_helperSimpleImage.textScale = 0;
					var emptyCharCount : int = simpleTextCount - numChars;
					for ( i = 0; i < emptyCharCount; ++i )
					{
						meshBatch.addMesh( _helperSimpleImage );
					}
				}
		...
		/** @inheritDoc */
		public function clearMeshBatch( meshBatch : IMeshBatch ) : void
		{
			meshBatch.clear();
		}
			...
			while ( !finished )
			{
				...
				if ( _lineHeight <= containerHeight )
				{
					...
					for ( i = 0; i < numChars; ++i )
					{
						...
						var char : BitmapChar = _chars[ charID ]; //getChar( charID );
		 ...
		 * 只支持单行文本
		 */
		private function arrangeSimpleChars( width : Number, height : Number, text : String,
			format : TextFormat, options : TextOptions ) : Vector.<CharLocation>
		{
			if ( text == null || text.length == 0 )
				return CharLocation.vectorFromPool();
			...
			var line : Vector.<CharLocation> = currentLine;
			numChars = line.length;
			if ( numChars > 0 )
			{
				var bottom : Number = _lineHeight;
				var yOffset : int = 0;
				...
				for ( var c : int = 0; c < numChars; ++c )
				{
					...
					var t : Texture = charLocation.char.texture;
