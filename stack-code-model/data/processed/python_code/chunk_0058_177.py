...
public class BasicAdjusterUI extends BaseComponentUI implements AdjusterUI{
	...
	{
		switch(key) {
			case "Adjuster.background":
				return new ASColor();
			break;
			case "Adjuster.foreground":
				return new ASColor(0xE1E2D6, 0.4);
			break;
			case "Adjuster.mideground":
	...
	protected function createArrowIcon() : Icon {
		return new ArrowIcon(Math.PI/2, 17);
	}
	...
	private function __onArrowButtonPressed(e:Event):void{
		...
		// MY!!!!!!!
		agp.move(windowP.x, windowP.y+adjuster.getHeight()*.5);
...
import org.aswing.Icon;
	...
	/* INTERFACE devoron.aswing3d.Icon */
	...
	{
		g.beginFill(new SolidBrush(new ASColor(0x000000, 0.4)));
		g.drawPolygon(new Pen(new ASColor(0xFFFFFF, 0.5)), new Array(new Point(0, 6), new Point(10, 6), new Point(5, 12)));
		g.endFill();
	}
