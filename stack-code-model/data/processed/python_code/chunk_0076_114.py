...
package devoron.aswing3d.plaf.basic
{
	...
	public class BasicColorSwatchesUI extends BaseComponentUI implements ColorSwatchesUI
	{
		...
		private function installListeners():void
		{
			...
			colorTilesPane.doubleClickEnabled = true;
			colorTilesPane.addEventListener(MouseEvent.DOUBLE_CLICK, __colorTilesPaneDoubleClick);
		...
		private function uninstallListeners():void
		{
			...
			colorTilesPane.doubleClickEnabled = false;
			colorTilesPane.removeEventListener(MouseEvent.DOUBLE_CLICK, __colorTilesPaneDoubleClick);
		...
		private function __colorTilesPaneDoubleClick(e:MouseEvent):void
		{
		...
		{
			return [0x000000, 0x333333, 0x666666, 0x999999, 0xCCCCCC, 0xFFFFFF, 0xFF0000, 0x00FF00, 0x0000FF, 0xFFFF00, 0x00FFFF, 0xFF00FF];
		}
			...
			for (var t:Number = 0; t < 6; t++)
			{
				for (var x:Number = 0; x < 6; x++)
				{
					for (var y3:Number = 0; y3 < 6; y3++)
					{
						...
						fillRect(g, startX + (t % 3) * (6 * L) + x * L, startY + Math.floor(t / 3) * (6 * L) + y3 * L, c);
					}
				}
			}
		}
		...
		{
			return new ColorRectIcon(38, 21, colorSwatches.getSelectedColor());
		}
		
		private function createHexText():JTextField
		{
			return new JColorTextField("#FFFFFF", 6);
		}
