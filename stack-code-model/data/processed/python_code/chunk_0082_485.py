...
package starling.utils
{
	...
	public class RectangleUtil
	{
			...
			out : Rectangle = null ) : Rectangle
		{
			...
			out.setTo(
				into.x + ( into.width - width ) * 0.5,
				into.y + ( into.height - height ) * 0.5,
				width, height );
...
//                MatrixUtil.transformCoords(matrix, positions[i].x, positions[i].y, sPoint);
				...
				point = positions[ i ];
				px = matrix.a * point.x + matrix.c * point.y + matrix.tx;
				py = matrix.d * point.y + matrix.b * point.x + matrix.ty;
			...
			out : Vector.<Point> = null ) : Vector.<Point>
		{
			...
			var rx:Number = rectangle.x;
			var ry:Number = rectangle.y;

			out[ 0 ].x = rx;
			out[ 0 ].y = ry;
			out[ 1 ].x = rx + rectangle.width;
			out[ 1 ].y = ry;
			out[ 2 ].x = rx;
			out[ 2 ].y = ry + rectangle.height;
			out[ 3 ].x = rx + rectangle.width;
			out[ 3 ].y = ry + rectangle.height;
			return out;
		}
