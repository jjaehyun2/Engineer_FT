package entities 
{
	...
	/**
	 * Spike Mutant Hostile
	 * Attacks from melee and shoots spikes in range
	 * @author Joao Borks
	 */
	public class Spike extends Sprite
	{
		...
		public function Spike(spawnX:int, spawnY:int, faceBack:Boolean = true) 
		{
			super();
			// Position
			...
			faceAhead = !faceBack;
			// Collision bounds
			_myBounds = new Quad(80, 130, 0x00FF00);
			_myBounds.alignPivot("center", "bottom");
			_atkBounds = new Quad(30, 80, 0xFF0000);
		...
		private function playerRange(e:EnterFrameEvent = null):void
		{
			...
			animSet.playAnim("sp_still", true);
			...
			if (distance < stage.stageWidth - 200 && player.life > 0)
			{
				...
				{
					// Move with a chance to attack from range
		...
		// Fire 5 shards with a 45 degree difference each
		private function fireShards():void 
		{
			var refY:int = y - 3 * height / 4;
			// Shard creation
			parent.addChild(new Shard(x, refY, "<"));
			parent.addChild(new Shard(x, refY, "<^"));
			parent.addChild(new Shard(x, refY, "^"));
			parent.addChild(new Shard(x, refY, "^>"));
			parent.addChild(new Shard(x, refY, ">"));
