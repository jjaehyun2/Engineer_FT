package Bezel.GCFW 
{
	...
	public class GCFWBezel implements MainLoader
	{
		...
		bezel_internal function stageKeyDown(e: KeyboardEvent): void
		{
			...
			{
				var msRemaining:int = 10 * 1000 + bezel.modsReloadedTimestamp - getTimer();
				if (msRemaining > 0)
				{
					var secondsRemaining:int = Math.ceil(msRemaining / 1000);
					GV.vfxEngine.createFloatingText4(GV.main.mouseX,GV.main.mouseY < 60?Number(GV.main.mouseY + 30):Number(GV.main.mouseY - 20),"Please wait " + secondsRemaining.toString() + (secondsRemaining > 1 ? " seconds!" : " second!"),16768392,14,"center",Math.random() * 3 - 1.5,-4 - Math.random() * 3,0,0.55,12,0,1000);
					return;
				}
