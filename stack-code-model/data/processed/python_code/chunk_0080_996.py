...
public class DragManager
{
	...
	/*modified*/
    /*
	Глобальный диспачер событий DRAG_START и DRAG_COMPLETE
	*/
    ...
    public static function doDrag(
			...
			dontProcessRelatedObjectEvents : Boolean = true ) : void
    {
        impl.doDrag(dragInitiator, dragSource, mouseEvent, dragImage, xOffset,
                yOffset, imageAlpha, allowMove, dontProcessRelatedObjectEvents );
		
		/*modified*/
		dragInitiator.addEventListener( DragEvent.DRAG_COMPLETE, onDragComplete );
		dispatcher.dispatchEvent( new DragEvent( DragEvent.DRAG_START, false, true, dragInitiator, dragSource ) );
		/*modified*/
    }
	/*modified*/
	private static function onDragComplete( e : DragEvent ) : void
	{
		e.dragInitiator.removeEventListener( DragEvent.DRAG_COMPLETE, onDragComplete );
