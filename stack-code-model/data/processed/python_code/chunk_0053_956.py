...
class ItemCard extends MovieClip
{
	...
	var ItemStabilityText: TextField;
	...
	function set itemInfo(aUpdateObj: Object): Void
	{
		...
		switch (_iItemType) {
			case Inventory.ICT_ARMOR:
				...
				var temp:Array = aUpdateObj.name.split("§");
		        aUpdateObj.name=temp[0];
		        ItemStabilityText.SetText(temp[1]);
				break;
			...
			case Inventory.ICT_INGREDIENT:
				...
				for (var i: Number = 0; i < 4; i++) {
					...
					if (aUpdateObj["itemEffect" + i] != undefined && aUpdateObj["itemEffect" + i] != "") {
						this["EffectLabel" + i].textColor = 0xF9EDD5 ;
		...
		if (aUpdateObj.name != undefined) {
			...
			ItemText.ItemTextField.textColor = aUpdateObj.negativeEffect == true ? 0xFF0000 : 0xF9EDD5 ;
		}
