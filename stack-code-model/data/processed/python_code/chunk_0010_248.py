...
*"* do not include other source files here!!!
  ...
  methods GET_COMMENTS_ITERATOR
    returning
      value(EO_ITERATOR) type ref to CL_OBJECT_COLLECTION_ITERATOR .
  methods ADD_VMLDRAWING
    importing
      !IP_VMLDRAWING type ref to ZCL_EXCEL_VMLDRAWING .
  methods GET_VMLDRAWING_ITERATOR
    returning
      value(EO_ITERATOR) type ref to CL_OBJECT_COLLECTION_ITERATOR .
  methods GET_VMLDRAWINGS
    returning
      value(R_VMLDRAWINGS) type ref to ZCL_EXCEL_VMLDRAWINGS .
...
*"* do not include other source files here!!!
  ...
  data VMLDRAWINGS type ref to ZCL_EXCEL_VMLDRAWINGS .
...
endmethod.
method ADD_VMLDRAWING.
  vmldrawings->include( ip_vmldrawing ).
endmethod.

method GET_VMLDRAWINGS.

    DATA: lo_vmldrawing  TYPE REF TO zcl_excel_vmldrawing,
        lo_iterator TYPE REF TO cl_object_collection_iterator.
    CREATE OBJECT r_vmldrawings.
     lo_iterator = vmldrawings->get_iterator( ).
    WHILE lo_iterator->has_next( ) = abap_true.
      lo_vmldrawing ?= lo_iterator->get_next( ).
      r_vmldrawings->include( lo_vmldrawing ).
    ENDWHILE.

endmethod.


method GET_VMLDRAWING_ITERATOR.

     eo_iterator = vmldrawings->get_iterator( ).
