...
package components.supportClasses
{
	...
	public class ScrollerLayout extends LayoutBase
	{
		...
		{  
			...
			if (hsbVisible)
			{
				...
				hsb.setLayoutBoundsSize(Math.max(hsb.getMinBoundsWidth(), hsbW) - scroller.hScrollBarRight, hsbH);
				
				hsb.setLayoutBoundsPosition( scroller.hScrollBarLeft, h - hsbH);
			}
			...
			if (vsbVisible)
			{
				...
				vsb.setLayoutBoundsSize(vsbW, Math.max(vsb.getMinBoundsHeight(), vsbH) - scroller.vScrollBarBottom - scroller.vScrollBarTop );
				
				vsb.setLayoutBoundsPosition(w - vsbW, scroller.vScrollBarTop);
			}
