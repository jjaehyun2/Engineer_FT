...
package com.neopets.games.inhouse.HideTheFart
{
	...
	import com.neopets.games.inhouse.HideTheFart.TranslationGameEngine; // translation probably not needed for this game
	...
	public class HideTheFartEngine_Document extends NP9_DocumentExtension
	{
		...
		private var mHideTheFartEngine:HideTheFartEngine;
		...
		public function HideTheFartEngine_Document():void{
		...
		{
			if (evt.oData.CMD == mHideTheFartEngine.GAME_ENGINE_CLEANED)
			{
		...
		**/
		...
		protected override function gameEngineUpdate():void
		{
			...
			mHideTheFartEngine.init(this,mcBIOS.finalPathway, mTranslationData);	
		...
		{
			mHideTheFartEngine = new HideTheFartEngine();
			mHideTheFartEngine.addEventListener(mHideTheFartEngine.SEND_THROUGH_CMD,sendThroughEvent,false,0,true);	
			...
			mcBIOS.game_id =  1264; 														//This Number will be provided by Neopets, For Demo it's 13000, GameEngineTest is 1105
			...
			// Changed images50 for working remotely
