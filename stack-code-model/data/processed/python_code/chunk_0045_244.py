...
class MemberListLine extends Sprite {
    ...
    function MemberListLine(place:int, name:String, rank:int, fame:int, isMe:Boolean, myRank:int, lastSeen:int) {
        ...
        var playerTime:String = this.getLastSeen(playerDate,myTimeDate,isMe);
    ...
    private function getLastSeen(playerDate:Date, myTimeDate:Date, isMe:Boolean) : String {
        ...
        var result:int = 0;
        var resultMinutes:int = 0;
        if(isMe) {
            return "Online";
        }
        if(myTimeDate.getFullYear() > playerDate.getFullYear()) {
            resultYear = myTimeDate.getFullYear() - playerDate.getFullYear();
            return resultYear > 1?resultYear + " years ago":resultYear + " year ago";
        }
        if(myTimeDate.getMonth() > playerDate.getMonth()) {
            resultMonth = myTimeDate.getMonth() - playerDate.getMonth();
            return resultMonth > 1?resultMonth + " months ago":" month ago";
        }
        if(myTimeDate.getHours() > playerDate.getHours()) {
            result = myTimeDate.getHours() - playerDate.getHours();
            return result > 1?result + " hours ago":result + " hour ago";
        }
        if(myTimeDate.getMinutes() > playerDate.getMinutes()) {
            resultMinutes = myTimeDate.getMinutes() - playerDate.getMinutes();
