package tetragon.view.render2d.extensions.graphics
{
	...
	public class Fill2D extends Graphic2D
	{
		public static const VERTEX_STRIDE:int = 9;
		private var fillVertices:VertexList2D;
		private var _numVertices:int;


		public function Fill2D()
		{
			clear();
		...
		public function clear():void
		{
			...
			_numVertices = 0;
			VertexList2D.dispose(fillVertices);
			fillVertices = null;
			isInvalid = true;
		}
		...
		public function addVertex(x:Number, y:Number, color:uint = 0xFFFFFF, alpha:Number = 1):void
		{
			...
			var node:VertexList2D = VertexList2D.getNode();
			if ( _numVertices == 0 )
			{
				fillVertices = node;
				node.head = node;
		 ...
		 * 
		 */
		private static function triangulate(vertices:VertexList2D, _numVertices:int, outputVertices:Vector.<Number>, outputIndices:Vector.<uint>):void
		{
			vertices = VertexList2D.clone(vertices);
			var openList:Vector.<VertexList2D> = convertToSimple(vertices);
			flatten(openList, outputVertices);

			while ( openList.length > 0 )
			{
				var currentList:VertexList2D = openList.pop();

				if ( isClockWise(currentList) == false )
				{
					VertexList2D.reverse(currentList);
				}

				var iter:int = 0;
				var flag:Boolean = false;
				var currentNode:VertexList2D = currentList.head;
				while ( true )
				{
					if ( iter > _numVertices * 3 ) break;
					iter++;

					var n0:VertexList2D = currentNode.prev;
					var n1:VertexList2D = currentNode;
					var n2:VertexList2D = currentNode.next;
					...
					if ( n2.next == n0 )
					{
						...
						VertexList2D.releaseNode(n0);
						VertexList2D.releaseNode(n1);
						VertexList2D.releaseNode(n2);
						break;
					}
					...
					var startNode:VertexList2D = n2.next;
					var n:VertexList2D = startNode;
				...
				VertexList2D.dispose(currentList);
			}
		}
		 ...
		 * @return 
		 */
		private static function convertToSimple(vertexList:VertexList2D):Vector.<VertexList2D>
		{
			var output:Vector.<VertexList2D> = new Vector.<VertexList2D>();
			var outputLength:int = 0;

			var openList:Vector.<VertexList2D> = new Vector.<VertexList2D>();
			...
			while ( openList.length > 0 )
			{
				...
				var headA:VertexList2D = currentList.head;
				var nodeA:VertexList2D = headA;
				...
				do
				{
					var nodeB:VertexList2D = nodeA.next.next;
					do
					{
						...
						if ( isect != null )
						{
							...
							var isectNodeA:VertexList2D = VertexList2D.getNode();
							...
							var headB:VertexList2D = nodeB;
							var isectNodeB:VertexList2D = VertexList2D.getNode();
		...
		private static function flatten(vertexLists:Vector.<VertexList2D>, output:Vector.<Number>):void
		{
			var L:int = vertexLists.length;
			var index:int = 0;
			for ( var i:int = 0; i < L; i++ )
			{
				var vertexList:VertexList2D = vertexLists[i];
		...
		private static function windingNumberAroundPoint(vertexList:VertexList2D, x:Number, y:Number):int
		{
			var wn:int = 0;
			var node:VertexList2D = vertexList.head;
			do
		...
		public static function isClockWise(vertexList:VertexList2D):Boolean
		{
			var wn:Number = 0;
			var node:VertexList2D = vertexList.head;
			do
		...
		static private function intersection(a0:VertexList2D, a1:VertexList2D, b0:VertexList2D, b1:VertexList2D):Vector.<Number>
		{
