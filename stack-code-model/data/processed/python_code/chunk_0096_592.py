		...
		bezel_internal function stageKeyDown(e: KeyboardEvent): void
		{
			if (this.bezel.keybindManager.getHotkeyValue("GCCS Bezel: Reload all mods").matches(e))
			{
				...
				if (msRemaining > 0)
				{
					...
					GV.vfxEngine.createFloatingText(GV.main.mouseX,GV.main.mouseY < 60?Number(GV.main.mouseY + 30):Number(GV.main.mouseY - 20),"Please wait " + secondsRemaining.toString() + (secondsRemaining > 1 ? " seconds!" : " second!"),16768392,14,"center",Math.random() * 3 - 1.5,-4 - Math.random() * 3,0,0.55,12,0,1000);
					return;
				}
