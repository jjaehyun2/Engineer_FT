...
CLASS zcl_abapgit_ci_repo IMPLEMENTATION.
  ...
  METHOD check_leftovers.
    ...
    LOOP AT lt_tadir ASSIGNING <ls_tadir> WHERE object = 'SOTR'.
      SELECT COUNT(*) FROM sotr_head INTO lv_count WHERE concept = <ls_tadir>-obj_name.
      IF sy-subrc = 0.
        lv_sotr = lv_sotr + lv_count.
      ENDIF.
    ENDLOOP.
    IF lv_sotr > 0.
      zcx_abapgit_exception=>raise( |Left over short texts: { lv_sotr }| ).
    ENDIF.

    LOOP AT lt_tadir ASSIGNING <ls_tadir> WHERE object = 'SOTS'.
      SELECT COUNT(*) FROM sotr_headu INTO lv_count WHERE concept = <ls_tadir>-obj_name.
      IF sy-subrc = 0.
        lv_sots = lv_sots + lv_count.
      ENDIF.
    ENDLOOP.
    IF lv_sots > 0.
      zcx_abapgit_exception=>raise( |Left over long texts: { lv_sotr }| ).
    ENDIF.
